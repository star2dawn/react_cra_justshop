...
function Section2Component ({notice, gallery, modalOpen}){
  ...
  //   // 공지사항 클릭 화살표함수 매개변수 2개를 전달 받을 때 이벤트
...

              {
                notice.map((item,idx)=>{
                  return(
                    ...
                    idx===0?
                    <li key={idx}><a href="!#" onClick={onClickModal} className='modal-btn'>{item.공지글목록}</a><span>{item.날짜}</span></li>
                    :
                    <li key={idx}><a href="!#">{item.공지글목록}</a><span>{item.날짜}</span></li>
                  )
